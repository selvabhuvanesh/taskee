...
struct ContentView: View {
    ...
    private func fetchDailyMotivation() {
        ...
        let monthTasks = tasks.filter { $0.assignedTo == authManager.userName && !$0.isArchived && $0.targetDate >= startOfMonth }
...
struct DateTasksView: View {
    ...
    private func performRankBonusCheck() {
        let memberToCheck = memberName.isEmpty ? authManager.userName : memberName
        let quest = MonthlyQuest.compute(tasks: allTasks, userName: memberToCheck)
        guard let bonus = RankBonusChecker.check(quest: quest, userName: memberToCheck) else { return }
        if let me = allMembers.first(where: { $0.name == memberToCheck }) {
            me.addReward(bonus.coins)
            let familyCode = authManager.familyCode
            Task { await cloudKitManager.pushMember(me, familyCode: familyCode) }
        }
    ...
    private func handleApproval(task: Item) {
        ...
        showCelebration = true
        // Check rank bonus after approval (delayed to let celebration finish)
        Task { @MainActor in
            try? await Task.sleep(nanoseconds: 3_000_000_000)
            self.performRankBonusCheck()
        }
    }
